...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	protected function initializeBasketOrderIfNotExists(Sale\Basket $basket)
	{
		if (!$basket->getOrder())
		{
			$serviceUserID = 109551;

			$userId = $this->getUserId() ?: CSaleUser::GetAnonymousUserID();
			$order = Sale\Order::create($this->getSiteId(), $serviceUserID);
	...
	protected function processBasketItem(Sale\BasketItem $item)
	{
		...
		// $basketItem['CUSTOM_PRICE'] = PriceMaths::roundPrecision($basketItem['CUSTOM_PRICE']);
		// $basketItem['CUSTOM_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['CUSTOM_PRICE'], $basketItem['CURRENCY'], true);
