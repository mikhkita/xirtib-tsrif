...
?>
	...
	$arJsonPrice = CIBlockElement::GetProperty($arBasePrice['ELEMENT_IBLOCK_ID'],$arBasePrice['PRODUCT_ID'], array(), Array("CODE"=>"JSON_PRICE"))->GetNext();
	$arJsonPrice['VALUE'] = json_decode(str_replace('&quot;', '"', $arJsonPrice['VALUE']), true);

	if ($arJsonPrice['VALUE']) {
		foreach ($arJsonPrice['VALUE'] as $item) {
			if ($item['ID'] == $arBasePrice['ID']) {
				$arBasePrice['PRICE'] = $item['PRICE'];
			}
		}
	}
