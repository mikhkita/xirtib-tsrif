	...
	<? foreach ($arResult['ITEMS'] as $key => $arItem): ?>
		...
		<? if ($print): ?>
			<?if($arRes = $res->GetNext()):?>
				<div class="b-tab <?=$actvie?>" data-tab="<?=$arParams['LIST_ID']?>-<?=$arRes['CODE']?>">
					<span><?=$arRes['NAME']?></span>
				</div>

				<? $arCategories[$arRes['ID']] = array(); ?>
				<? $arCategories[$arRes['ID']]['NAME'] = $arRes['NAME']; ?>
				<? $arCategories[$arRes['ID']]['CODE'] = $arParams['LIST_ID'].'-'.$arRes['CODE']; ?>
...
<? foreach ($arCategories as $sectionID => $category): ?>
	<? $hide = (intval($arResult['ITEMS'][0]['IBLOCK_SECTION_ID']) !== intval($sectionID)) ? 'hide' : ''; ?>
	<div class="b-tab-item b-catalog-list <?=$hide?>" id="<?=$category['CODE']?>">
		<? foreach ($arResult['ITEMS'] as $key => $arItem): ?>
			<? if($arItem['IBLOCK_SECTION_ID'] == $sectionID && $key < 4): ?>
				<? $priceText = ''; ?>
				<? $isQuantity = false; ?>
...
<? if(count($arResult["ITEMS"])): ?>
	<div class="b-catalog-list b-tab-item" id="<?=$arParams["LIST_ID"]?>-">
