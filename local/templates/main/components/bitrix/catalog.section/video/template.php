...
<? if(count($arResult["ITEMS"])): ?>
	<div class="b-block">
		...
		<?foreach($arResult["ITEMS"] as $key => $arItem):?>
			...
			?>
			<div class="b-catalog-item <? if( $arItem["PRICES"]["PRICE"]["DISCOUNT_VALUE"] != $arItem["PRICES"]["PRICE"]["VALUE"] ): ?>discount-item<?endif;?> clearfix<? if( !$arItem["CATALOG_QUANTITY"] ): ?> with-notice<? endif; ?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
				<div class="b-catalog-back"></div>
				<a data-fancybox href="<?=$arParams["LINKS"][$key]?>" class="b-catalog-img video-link" style="background-image:url('<?=$renderImage["src"]?>');">
					...
					<div class="catalog-item-discount icon-discount-full"><p>-<?=$percent?>%</p></div>
					<?endif;?>
				</a>
						...
						<div class="price-container">
							<? if( $arItem["PRICES"]["PRICE"]["DISCOUNT_VALUE"] != $arItem["PRICES"]["PRICE"]["VALUE"] ): ?>
								<div class="b-discount-price">
									...
									<div class="new-price pink icon-rub"><?=number_format( $arItem["PRICES"]["PRICE"]["DISCOUNT_VALUE"], 0, ',', ' ' )?></div>
							...
							<? if( count($arItem["ITEM_PRICES"]) > 1 ): ?>
								<? foreach ($arItem["ITEM_PRICES"] as $kp => $price): ?>
									...
									<div class="b-discount-price b-dynamic-price b-dynamic-discount-price" style="display:none;" data-from="<?=$price["QUANTITY_FROM"]?>">
										<div class="old-price icon-rub"><?=number_format( $arItem["PRICES"]["PRICE"]["VALUE"], 0, ',', ' ' )?></div>
										<div class="new-price pink icon-rub"><?=number_format( $price["PRICE"], 0, ',', ' ' )?></div>
									</div>
								<? endforeach; ?>
							<? endif; ?>
						</div>
