;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		getDiscountObjects: function(result){
			var discounts = [],
				list = result.APPLIED_DISCOUNT_LIST;
			...
			for( var i in result.TOTAL_RENDER_DATA.COUPON_LIST ){
				...
				if( coupon.ID == 3 ){
					var sum = 0,
						discount = list[coupon.DISCOUNT_ID];
					...
					for( var k in discount.ACTIONS.CHILDREN ){
						var action = discount.ACTIONS.CHILDREN[k];

						if( action.DATA.Type == "Discount" ){
							if( action.DATA.Unit == "Perc" ){
								sum = action.DATA.Value+"%";
							}else{
								sum = action.DATA.Value+" руб."
							}
						}
					}

					discounts.push({
						title : "Скидка за отзывы",
						sum : sum
					});
					break;
				}
			...
			for( var i in list ){
				var discount = list[i];
				// console.log(list[i]);
				for( var j in discount.CONDITIONS.CHILDREN ){
					var item = discount.CONDITIONS.CHILDREN[j];
					if( item.CLASS_ID == "CondMainUserId" ){
						var sum = "0";
						for( var k in discount.ACTIONS.CHILDREN ){
							var action = discount.ACTIONS.CHILDREN[k];
		...
		showDiscounts: function(result){
			var discounts = this.getDiscountObjects(result),
				str = "";

			for( var i in discounts ){
				var discount = discounts[i];
				str += ("<p>"+discount.title+": <b>"+discount.sum+"</b></p>");
			}

			result.TOTAL_RENDER_DATA.DISCOUNT_LIST = str;


			// if( document.getElementById("b-discount-list") ){
				// document.getElementById("b-discount-list").html = str;
			// }
			// console.log(str);
		},
